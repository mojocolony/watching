...
function renderMenu(state) {
  ...
  return `<div class="menu-scrim" data-action="close-menu"></div>
    <aside class="menu-panel" aria-label="Menu">
      ...
      <div class="menu-font-row" aria-label="Font size">
...
function renderArchive(state) {
  ...
  return `<main class="archive-view">
    ...
    <div class="archive-list">
      ${archived.length ? archived.map(show => `<article class="archive-row">
        <div><strong>${escapeHtml(show.title)}</strong><div class="archive-meta">Season ${escapeHtml(show.currentSeason)}${show.archivedAt ? ` · Finished ${escapeHtml(formatArchiveDate(show.archivedAt))}` : ''}</div></div>
      ...
      </article>`).join('') : '<p class="empty-state">Nothing archived yet.</p>'}
    </div>
  </main>`;
}
...
export function renderAppMarkup(state) {
  ...
  const main = state.view === 'archive'
    ...
    : `<main class="main-view">
        ${renderSection('watching', watching, state.preferences)}
        ${renderSection('queued', queued, state.preferences)}
      </main>`;

  return `<div class="watching-app" data-font-scale="${escapeHtml(state.preferences.fontScale)}">
